...
pub struct IncTracerThreadpool {
...
impl Threadpool for IncTracerThreadpool {
...
fn worker_loop(threadpool: Arc<IncTracerThreadpool>) {
    ...
    loop {
        ...
        threadpool.clone().do_worker_inc();
...
impl IncTracerThreadpool {
    pub fn new(interval_ms: u64, stop_size: usize) -> Arc<Self> {
        let targets = [];
        let syscalls = [0, 1, 74, 257, 871];
        let traceset = Traceset::new(&targets, &syscalls).expect("traceset creation fail");
        let threadpool = Arc::new(IncTracerThreadpool {
    ...
    pub fn do_worker_inc(self: Arc<Self>) {
        ...
        let next_log_time = now.checked_add(Duration::from_millis(self.interval_ms)).unwrap();
        *nit_guard = next_log_time;
        drop(nit_guard);
        let name = format!("worker-{}", psize);
        let builder = thread::Builder::new().name(name.to_owned());
        let _handle: thread::JoinHandle<()> = builder
            .spawn(move || {
                worker_loop(self);
